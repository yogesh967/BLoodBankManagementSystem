...
header('location:index.php');
	...
	$status="2";
	$sql = "UPDATE bbregister SET Status=:status WHERE ID=:eid";
	$query = $dbh->prepare($sql);
	$query -> bindParam(':status',$status, PDO::PARAM_STR);
	$query-> bindParam(':eid',$eid, PDO::PARAM_STR);
	$query -> execute();

	$msg="Blood Bank Hidden Successfully";
	}
	...
	$msg="Blood Bank public Successfully";
	}
	if(isset($_REQUEST['del']))
		{
	$did=intval($_GET['del']);
	$sql = "DELETE from bbregister WHERE ID=:did";
	$query = $dbh->prepare($sql);
		...
		<div class="content-wrapper">
			<div class="container-fluid">
                ...
                <table id="zctb" class="table table-bordered table-striped table-hover" cellspacing="0" width="100%">
                  ...
                  <?php $sql = "SELECT * from bbregister where status=1 OR status=2 ORDER BY ID";
                  $query = $dbh -> prepare($sql);
                  $query->execute();
